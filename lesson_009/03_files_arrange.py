...
class FileOrder:
        ...
        for dirpath, dirnames, filenames in os.walk(self.source_path):
            for file_name in filenames:
                ...
                if extension in self.image_extension:
                    ...
                    struct_path = os.path.join('icons_by_year', str(struct_file_date.tm_year),
                                               str(struct_file_date.tm_mon))
                    os.makedirs(struct_path, exist_ok=True)
                    recipient_full_path = os.path.join(struct_path, file_name)
    ...
    def archive_sort(self):
        ...
        for file_name in source_zip.infolist():
            ...
            if extension in self.image_extension:
                target_path = os.path.join('icons_by_year', str(file_name.date_time[0]), str(file_name.date_time[1]))
                target_file = os.path.join(target_path, os.path.split(name)[1] + extension)
